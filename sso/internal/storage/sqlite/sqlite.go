...
func (s *Storage) User(ctx context.Context, email string) (models.User, error) {
	...
	stmt, err := s.db.Prepare("SELECT id, email,username, pass_hash FROM users WHERE email = ?")
	if err != nil {
	...
	err = row.Scan(&user.Id, &user.Email, &user.Name, &user.PassHash)
	if err != nil {
...
func (s *Storage) IsAdmin(ctx context.Context, userId int64) (bool, error) {
	...
	usr, err := s.db.Prepare("select is_admin from users where id = ?")
	if err != nil {
		return false, fmt.Errorf("%s: %w", op, err)
	}

	row := usr.QueryRowContext(ctx, userId)
	var isAdmin bool
